...
edgesToVertices inp = [
        deref $ shift (nv2e 0) inp,
        deref $ shift (nv2e 1) inp,
        deref $ shift (nv2e 2) inp,
        deref $ shift (nv2e 3) inp
    ]
...
tests = [
        testGroup "basic" [
            ...
            testCase "shift" $ deref (shift (nv2v 0) vItOnV) @?= 1,
            testCase "lift1" $ deref (lift1 deref vItOnV) @?= 0,
            testCase "lift1 + shift" $ deref (shift (nv2v 0) (lift1 deref vItOnV)) @?= 1,
            testCase "lift2" $ deref (lift2 stencil2 vItOnV vItOnV) @?= 0,
            testCase "lift2 + shift" $ deref (shift (nv2v 0) (lift2 stencil2 vItOnV vItOnV)) @?= 2,
            testCase "nbShift" $ deref (nbShift v2v vItOnV) @?= [1, 3, 2, 6],
            testCase "nbShift" $ deref (shift (nv2v 0) (nbShift v2v vItOnV)) @?= [2, 3, 0, 7]
        ],
        testGroup "edgesToVertices" [
            ...
            testCase "lifted + shifted neighbor sum" $ deref (shift (nv2v 0) (lift1 (reduce1 (+) 0) (nbShift v2e eItOnV))) @?= 27
        ],
        testGroup "complex" [
